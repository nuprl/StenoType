...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        help="file for saving checkpoints")
    parser.add_argument(
        "--checkpoint-steps",
        type=int,
        default=1000,
    ...
    if args.filter:
        ...
        if invalid_filters:
            parser.print_usage()
            print("error: the following filters are not valid: "
                  f"{','.join(invalid_filters)}")
            print("       must be any of: "
                  f"{','.join([f for f in valid_filters])}")
            exit(2)
...
def load(from_hf: bool, dataset_dir: str, workers: int) -> Dataset | IterableDataset:
    if from_hf:
        ...
        return load_dataset("bigcode/the-stack-dedup",
...
def filter_parses(
    ...
    print("Parsing files")
    dataset = dataset.filter(lambda e: transform.is_valid_syntax(e["content"]),
                             num_proc=workers)
...
def self_contained(content: str) -> bool:
    ...
    EXPORT_RE2 = re.compile("export[^\w_$]\s*(function|class|interface|type|"
                            "enum|const|declare|default|async)")
    matches = any(line for line in content.splitlines()
...
def filter_typechecks(
    ...
    with futures.ProcessPoolExecutor(max_workers=workers) as executor:
        fs = [executor.submit(run_tsc, d["hexsha"], d["content"])
    ...
    typechecks = filtered.filter(lambda d: checkpoint[d["hexsha"]],
                                 num_proc=workers)
...
def loc_per_function(s: str) -> float:
    ...
    fun_loc = [count_loc(transform.node_to_str(n).strip().strip("{}"))
               for n in nodes]
...
def is_after_cutoff(example: dict[str, Any], cutoff: datetime) -> bool:
    stars_date = parsedate(
        example["max_stars_repo_stars_event_min_datetime"]
    )
    forks_date = parsedate(
        example["max_forks_repo_forks_event_min_datetime"]
    )
    issues_date = parsedate(
        example["max_issues_repo_issues_event_min_datetime"]
    )
...
def main():
    ...
    if args.cutoff:
        print(f"Filtering for files after the {args.cutoff.strftime('%Y-%m-%d')} "
              "cutoff", flush=True)
        dataset = dataset.filter(functools.partial(is_after_cutoff, cutoff=args.cutoff),
                                 num_proc=args.workers)
        print("Size after filtering:", len(dataset))
